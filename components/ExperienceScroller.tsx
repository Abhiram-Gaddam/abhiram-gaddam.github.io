...
export default function ExperienceScroller({ entries }: { entries: ExperienceEntry[] }) {
  ...
  return (
    <div ref={containerRef} style={{ height: `${entries.length * 90}vh` }} className="relative">
      {/* Used items-start on mobile so short content stays at the top */}
      <div className="sticky top-20 flex min-h-[75vh] items-start py-10 lg:items-center lg:py-0">
        
        {/* Changed to 2 columns on desktop: Timeline + Combined Content */}
        <div className="grid w-full items-start gap-8 lg:grid-cols-[auto_1fr] lg:items-center lg:gap-14">
...
// Merged the Text and the Photo Deck into one component
// Merged the Text and the Photo Deck into one component
function Slide({
  entry,
  index,
  ...
  return (
    <motion.div 
      style={{ opacity, y, visibility, pointerEvents }} 
      className="col-start-1 row-start-1 flex w-full flex-col gap-8 will-change-transform lg:flex-row lg:items-center lg:gap-14"
    >
      {/* Text Content */}
      ...
      {/* Photo Deck */}
      {/* Mobile: takes up 85% of screen width (max 320px) keeping it centered. Desktop: slightly larger fixed max-width. Removed fixed heights! */}
      <div className="relative mx-auto w-[85%] max-w-[320px] shrink-0 lg:mx-0 lg:w-[400px] lg:max-w-none">
        <PhotoDeck images={entry.images} orgId={entry.id} accent={accent} />
      </div>
    </motion.div>
  );
